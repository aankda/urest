...
import { MousePointer2, ShieldCheck, ShieldAlert, Globe, X, List, ArrowLeft } from 'lucide-react';
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
        ...
        if (tab?.url) {
          try {
            ...
            if (url.protocol === 'chrome:' || url.protocol === 'about:' || url.protocol === 'moz-extension:' || url.protocol === 'chrome-extension:') {
            ...
            setIsExcluded(excluded.includes(host));
  ...
  return (
    <div className="p-4 bg-slate-900 min-h-[400px] flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="glass-card flex items-center justify-between group hover:border-brand-500/30">
          ...
          <button 
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all focus:outline-none ${isEnabled ? 'bg-brand-500 shadow-[0_0_15px_rgba(107,142,247,0.4)]' : 'bg-slate-700'}`}
          >
      ...
      <footer className="mt-auto pt-4 flex items-center justify-center gap-6 text-[10px] text-slate-500 font-bold uppercase tracking-widest">
        <a href="#" className="hover:text-brand-400 transition-colors">Feedback</a>
